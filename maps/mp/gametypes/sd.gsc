...
main()
{
	...
	maps\mp\gametypes\_globallogic_utils::registerRoundSwitchDvar( level.gameType, 0, 0, 1500 );
	...
	maps\mp\gametypes\_globallogic_utils::registerRoundLimitDvar( level.gameType, 0, 0, 1500 );
	maps\mp\gametypes\_globallogic_utils::registerRoundWinLimitDvar( level.gameType, 0, 0, 1500 );
...
onPrecacheGameType()
{
	game["bomb_dropped_sound"] = "mpl_flagdrop_sting_friend";
	game["bomb_recovered_sound"] = "mpl_flagget_sting_friend";
...
onUsePlantObject( player )
{
	if ( !self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		...
		for ( i = 0; i < level.players.size; i++ )
		{
			level.players[i] playLocalSound("xkmod_planted");
		}
...
onUseDefuseObject( player )
{
	...
	thread playSoundOnPlayers( "mus_sd_defused"+"_"+level.teamPostfix[player.pers["team"]] );
	
	for ( i = 0; i < level.players.size; i++ )
	{
		level.players[i] playLocalSound("xkmod_defused");
	}	
...
onPickup( player )
{
	...
	thread playSoundOnPlayers( game["bomb_recovered_sound"], game["attackers"] );
