...
//		player maps\mp\gametypes\_persistence::statAdd( "match_bonus", int( player.pers["summary"]["match"] ), false );
//		player maps\mp\gametypes\_persistence::statAdd( "misc_bonus", int( player.pers["summary"]["challenge"] + player.pers["summary"]["misc"] ), false );
...
displayRoundSwitch( winner, endReasonText )
{
	...
	thread xKMod\classmenu::updateClassAvailability( "allies" ); 
	thread xKMod\classmenu::updateClassAvailability( "axis" );
...
startGame()
{
	...
	if(!isDefined(game["timeout_over"]))
		game["timeout_over"] = false;
		
	if(game["timeout_called"])
	{
		xKMod\timeout::SaveScore();
		thread xKMod\timeout::main();
		level.inPrematchPeriod = false;
		return;
	}
	
	if( getDvar("g_gametype") == "sd" && ( getDvar("xk_mode") == "eslmatch" || getDvar("xk_mode") == "cevomatch" || getDvar("xk_mode") == "cbmatch" || getDvar("xk_mode") == "mymatch" ) )
	{
		if (xKMod\readyup::conditionReadyUp())
		{
...
Callback_StartGameType()
{
	...
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		level thread xKMod\_xkmod::doNoBob();
...
checkRoundSwitch()
{
	...
	{
		if ( game["is_match_mode"] && getDvar("xk_mode") != "cevomatch" )
		{
			game["ready_up"] = true;
			game["half_time"] = true;
			game["timeout_called"] = false;
			...
			for(i = 0; i < level.players.size; i++)
			{
				...
				player.pers["score_hold"] = player.pers["score"];
				player.pers["kills_hold"] = player.pers["kills"];
				player.pers["deaths_hold"] = player.pers["deaths"];
				player.pers["assists_hold"] = player.pers["assists"];
				player.pers["plants_hold"] = player.pers["plants"];
				player.pers["defuses_hold"] = player.pers["defuses"];
			}
		}
...
teamchangeGracePeriod()
{
	...
	{
/#
		PrintLn("teamchangeGracePeriod: abort\n");
#/		
	...
	{
/#
		PrintLn("teamchangeGracePeriod: start\n");
#/

		// allow teamchanging during graceperiod and balance the teams every balanceModulus seconds
		balanceModulus = 15;
		...
		while( level.teamchange_graceperiod > 0 )
		{
			...
			{
/#
				PrintLn("teamchangeGracePeriod: balance check\n");
#/			
				if( !maps\mp\gametypes\_teams::getTeamBalance() )
				{
/#
					PrintLn("teamchangeGracePeriod: balancing\n");
#/			
...
/#
		PrintLn("teamchangeGracePeriod: final balance check\n");
#/			
		if( !maps\mp\gametypes\_teams::getTeamBalance() )
		{
/#
			PrintLn("teamchangeGracePeriod: final balancing\n");
#/			
			maps\mp\gametypes\_teams::balanceTeams();
		}
	}
...
/#
		PrintLn("teamchangeGracePeriod: end\n");
#/	
	// grace period over
	level.teamchange_graceperiod = 0;
