...
init()
{
	...
	if ( level.teamBased )
	{
		...
		registerScoreInfo( "assist_75", 3 );
		registerScoreInfo( "assist_50", 3 );
		registerScoreInfo( "assist_25", 3 );
		registerScoreInfo( "assist", 3 );
		registerScoreInfo( "suicide", 0 );
		registerScoreInfo( "teamkill", 0 );
		registerScoreInfo( "dogkill", 0 );
		registerScoreInfo( "dogassist", 0 );
		registerScoreInfo( "helicopterkill", 0 );
		...
		registerScoreInfo( "spyplanekill", 0 );
		registerScoreInfo( "spyplaneassist", 0 );
		registerScoreInfo( "rcbombdestroy", 3 );
	...
	else
	{
		...
		registerScoreInfo( "spyplanekill", 0 );
		registerScoreInfo( "spyplaneassist", 0 );
		registerScoreInfo( "rcbombdestroy", 0 );
		registerScoreInfo( "plant", 3 );
		registerScoreInfo( "defuse", 3 );
	}
...
onPlayerConnect()
{
	for(;;)
	{
		...
		LOLUHAVENOLVL = player getRankForXp( 2516000 );
		player setRank( LOLUHAVENOLVL, 16 );
		player.pers["prestige"] = 16;
...
doStratTimer()
{
	timerText = self createFontString( "objective", 1.6 );
	timerText setPoint( "CENTER", "CENTER", 0, 0 );
	ready = self createFontString( "objective", 1.5 );
...
doStratTime()
{
	if(getDvar("xkmod_strattime") == "") 
		setDvar("xkmod_strattime", 6);
	
	setStratTime = getDvarInt("xkmod_strattime");
	
	level.stratTime = int(setStratTime);
    for(i = level.stratTime; i > 0; i--)
    {
		level.stratTime = i;
		wait 1;
