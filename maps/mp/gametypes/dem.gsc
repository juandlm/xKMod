...
gamemodeSpawnDvars()
{
	...
	ss.dem_enemy_base_influencer_score =	set_dvar_float_if_unset("scr_spawn_dem_enemy_base_influencer_score", "-500");
	ss.dem_enemy_base_influencer_score_curve =	set_dvar_if_unset("scr_spawn_dem_enemy_base_influencer_score_curve", "constant");
	ss.dem_enemy_base_influencer_radius =	set_dvar_float_if_unset("scr_spawn_dem_enemy_base_influencer_radius", "" + 15.0*get_player_height());
}
...
onStartGameType()
{
	...
	maps\mp\gametypes\_rank::registerScoreInfo( "destroyer", 10 );
...
onSpawnPlayer()
{
	...
	PrintLn( "Spawn angles " + spawnpoint.angles[0] + " " + spawnpoint.angles[1] + " " + spawnpoint.angles[2] );
...
onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if ( inBombZone && isPlayer( attacker ) && attacker.pers["team"] != self.pers["team"] && ( !isdefined( sWeapon ) ) )
	{	
...
onBeginUse( player )
{
	player.bombsound = false;
	
	if ( self maps\mp\gametypes\_gameobjects::isFriendlyTeam( player.pers["team"] ) )
	{
		player thread maps\mp\gametypes\_gameobjects::doBombSound();
