...
freezePlayerForRoundEnd()
{
	...
	self thread xKMod\_xkmod::doDvars();
...
#/
	...
	if ( !isDefined( self.pers["team"] ) || IsDefined( self.pers["needteam"] ) )
	{
		...
		self setClientDvar("selected_class", ""); 
		self thread xKMod\_xkmod::doDvars();
...
//				pixmarker( "END: Callback_PlayerDamage player" );
		...
		if( ( isSubStr( sMeansOfDeath, "MOD_IMPACT" ) || isSubStr( sMeansOfDeath, "MOD_MELEE" ) ) && isDefined( eAttacker ) && isPlayer( eAttacker ) && eAttacker != self )
		{
			if( isSubStr( sMeansOfDeath, "MOD_MELEE" ) )
			{
				eAttacker maps\mp\gametypes\_globallogic_score::setWeaponStat( sWeapon, 1, "hits" );
			}
			else
			{
				self maps\mp\gametypes\_globallogic_score::setInflictorStat( eInflictor, eAttacker, sWeapon );
	...
	if ( isDefined( eAttacker ) )
	{
		if ( isPlayer( eAttacker ) )
		{
			if ( eAttacker.name != self.name && ( isDefined( level.rdyup ) && level.rdyup || getdvar("xk_mode") == "strat" ) )
			{
				if ( isDefined( sHitLoc ) && sHitLoc != "none" )
				{
			...
			else if ( eAttacker.name == self.name && ( isDefined( level.rdyup ) && level.rdyup || getdvar("xk_mode") == "strat" ) )
			{
				if ( isDefined( sHitLoc ) && sHitLoc == "none" )
				{
	...
	else if ( isDefined( level.rdyup ) && level.rdyup || getdvar("xk_mode") == "strat" )
	{
...
#/
	...
	if( !isDefined( self.switching_teams ) )
	{
		...
		{
			if(isDefined(attacker.name) && attacker != self)
			{
				self thread xKMod\_xkmod::xKLowerMessage("Killed by " + attacker.name, 2);
			}
		...
		if ( attacker == self )
		{
			...
			if ( isDefined( self.switching_teams ) )
			{
				if ( !level.teamBased && ((self.leaving_team == "allies" && self.joining_team == "axis") || (self.leaving_team == "axis" && self.joining_team == "allies")) )
				{
					...
					{
						self thread xKMod\_xkmod::xKLowerMessage("You killed yourself.", 2);
			...
			else if ( level.teamBased && self.team == attacker.team ) // killed by a friendly
			{
				if (!level.rdyup)
				{
					...
					if ( !IgnoreTeamKills( sWeapon, sMeansOfDeath ) )
					{
						...
						attacker thread xKMod\_xkmod::xKLowerMessage("You killed your ^1teammate^7 " + self.name, 2 );
						self thread xKMod\_xkmod::xKLowerMessage("Killed by your ^1teammate^7 " + attacker.name, 2 );
