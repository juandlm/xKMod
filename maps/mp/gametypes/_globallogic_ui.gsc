...
menuAutoAssign()
{
	...
	if ( oldTeam != self.pers["team"] && ( oldTeam == "allies" || oldTeam == "axis" ) ) 
		thread xKMod\classmenu::updateClassAvailability( oldTeam );
...
menuClass( response )
{
	...
	if ( self.sessionstate == "playing" )
	{
		...
		isCompetitive = getDvar("xk_mode") == "eslmatch" || getDvar("xk_mode") == "cevomatch"|| getDvar("xk_mode") == "cbmatch" || getDvar("xk_mode") == "mymatch"; 
		supplyStationClassChange = isDefined( self.usingSupplyStation ) && self.usingSupplyStation;
		self.usingSupplyStation = false;

		if ( ( level.inGracePeriod && !self.hasDoneCombat ) || (level.gametype == "sd" && isCompetitive && game["ready_up"]) ) 
		{
		...
		else if ( !level.splitScreen )
		{
			...
			self iPrintlnBold( "Your class will change upon your next respawn.");
			self playlocalsound( "uin_alert_slideout" );
		}
	}
	else
	{
